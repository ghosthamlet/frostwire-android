...
import org.bouncycastle.crypto.engines.RC4EngineEx;
import org.bouncycastle.crypto.params.KeyParameterEx;
...
UDPConnectionSet 
{
	...
	private RC4EngineEx		header_cipher_out;
	private RC4EngineEx		header_cipher_in;
		...
		byte[]			session_secret )
	{
		try{
			if ( connection == lead_connection ){
			    ...
			    RC4EngineEx rc4_engine_a	= getCipher( a_key );	
	    		RC4EngineEx rc4_engine_b	= getCipher( b_key );
	    		RC4EngineEx rc4_engine_c	= getCipher( c_key );	
	    		RC4EngineEx rc4_engine_d	= getCipher( d_key );	
	...
	protected RC4EngineEx
	getCipher(
		byte[]			key )
	{
	    SecretKeySpec	secret_key_spec = new SecretKeySpec( key, "RC4" );
	    
	    RC4EngineEx rc4_engine	= new RC4EngineEx();
		
		CipherParametersEx	params_a = new KeyParameterEx( secret_key_spec.getEncoded());
